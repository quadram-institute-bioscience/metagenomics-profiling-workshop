...
[fastp](https://github.com/OpenGene/fastp) is a versatile and fast tool to perform quality filtering 
and trimming of sequencing reads.
...
At the beginning it can be useful to write the command in a text editor and substitute the input and output files.
After practicing manual executiona few times, we can learn how to automate the process using a `for` loop in bash. This is especially useful when dealing with *a lot of* samples, as it allows us to process all of them in a single command.
...
You can save the following as "filter-reads.sh" and then execute it with:

```bash
bash ./filter-reads.sh
```
...
RAW_READS=/shared/team/datasets/coffee/reads/
OUTDIR=./filt-reads/
